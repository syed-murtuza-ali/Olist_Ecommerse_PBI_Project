...
select 
(select round(sum(p.payment_value),0)
from orders o inner join payments p using(order_id) where  o.Weekend_Weekday="Weekend") as Weekend_count,
(select round(sum(p.payment_value),0)
from  orders o inner join payments p using(order_id) where o.Weekend_Weekday="Weekday") as Weekday_count;
